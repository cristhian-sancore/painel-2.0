...
async function main() {
  const res = await fetch(${PORTAINER_URL}/endpoints//docker/containers/json, {
    headers: { 'X-API-Key': TOKEN }
  });
  const containers = await res.json();
  const nextContainer = containers.find((c: any) => c.Image.includes('datacenter-panel'));
  if (nextContainer) {
    const logsRes = await fetch(${PORTAINER_URL}/endpoints//docker/containers//logs?stdout=true&stderr=true&tail=50, {
      headers: { 'X-API-Key': TOKEN }
    });
